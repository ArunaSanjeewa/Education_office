/**
 * Created by uer on 11/11/2017.
 */
'use strict';

angular.module('myApp.manage-occupation', ['ngRoute', 'ui.bootstrap'])

    .config(['$routeProvider', function ($routeProvider) {
        $routeProvider.when('/manage-occupation', {
            templateUrl: 'views/manage-occupation/manage-occupation.html',
            controller: 'manage-OccupationController'
        });
    }])
    .controller('manage-OccupationController',function ($http, $scope, filterFilter,$window) {
        var baseurl = '/api';
        ...
        function loadAllOccupation(){
            $http.get(baseurl + '/all_occupation').success(function (datafromRouter) {
                $scope.allItems = datafromRouter;
                $scope.data = datafromRouter;
                $scope.totalItems = $scope.data.length;
        ...
        $scope.createOccupation = function (valid) {

            if (!valid)
                return;

                     console.log("createOccupationcreateOccupationcreateOccupation")
            $http.post(baseurl + '/createoccupation', $scope.formData).success(function (response) {

                if (response.status == 1) {
                    dhtmlx.message({
                        type: "Information",
                        text: "<strong>Successfully Created</strong>",
                    });
                }
                    ...
                    if (response.data.code == 11000)
                        dhtmlx.message({
                            type: "error",
                            text: "<strong>Already Registered Supplier</strong>",
                        });
                }
        ...
        $scope.updateSupplier = function (valid) {

            if (!valid)
                return;

            $http.post(baseurl + '/update-supplier', { Supplier : $scope.updateData }).success(function (response) {

                if (response.status == 1) {
                    dhtmlx.message({
                        type: "Information",
                        text: "<strong>Successfully Updated</strong>",
                    });
                }
                else if (response.status == 0) {
                    if (response.data.code == 11000)
        ...
        $scope.removeSupplier = function (supplier) {
            $http.delete(baseurl + '/supplier', { params: { 'id': supplier._id } }).success(function (response) {

                dhtmlx.message({
                    type: "Information",
                    text: "<strong>Successfully Deleted</strong>",
                    // expire: -1
                });
